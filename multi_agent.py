...
class MultiAgent:
    def __init__(self, s_start, s_goal, step_len, goal_sample_rate, iter_max,V1 = None):
        ...
        self.env = env.Env()
        self.plotting = plotting.Plotting(s_start, s_goal,type = "multi")
    ...
    def extract_path(node_new, node_new_prim):
        ...
        while node_now.parent is not None:
            ...
            pathx1.append(node_now)

        path2 = [(node_new_prim.x, node_new_prim.y)]
        pathx2 = [node_new_prim]
        node_now = node_new_prim

        while node_now.parent is not None:
            node_now = node_now.parent
            path2.append((node_now.x, node_now.y))
            pathx2.append(node_now)

        return (list(list(reversed(path1)) + path2),list(list(reversed(pathx1))+pathx2))
...
def main():
    ...
    goal_list = [(37,18),(10,27),(30,27)]
    multi_plot = plotting.Plotting(x_start,goal_list,"multi")
    V1 = None
    true_path = []
    for i in goal_list:
        rrt_conn = MultiAgent(x_start, i, 0.8, 0.05, 5000,V1)
        path,pathx = rrt_conn.planning()
        true_path=path
        multi_plot.animation_connect(rrt_conn.V1,rrt_conn.V2,true_path,"Multi Agent")
        if V1 == None:
            V1 = pathx
        else:
            V1+=pathx

    #rrt_conn = MultiAgent(x_start, x_goal, 0.8, 0.05, 5000)
    #path,pathx = rrt_conn.planning()

    #rrt_conn.plotting.animation_connect(rrt_conn.V1, rrt_conn.V2, path, "Multi Agent")

    #rrt_conn = MultiAgent((18,8), (10,27), 0.8, 0.05, 5000,V1 = pathx)
    #path,patht = rrt_conn.planning()
